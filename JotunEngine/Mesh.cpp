...
bool Mesh::import( std::string filename ) {
	scene = aiImportFile( filename.c_str(), aiProcessPreset_TargetRealtime_Fast | aiProcess_FlipUVs );
	return scene != NULL;
}
...
void Mesh::normalizeUVs() {
	if( scene->mMeshes[0]->mTextureCoords[0] == NULL ) {
		return;
	}
	aiVector3D curTexCoord;
	for( int t = 0; t < scene->mMeshes[0]->mNumFaces; ++t ) {
		const struct aiFace* face = &scene->mMeshes[0]->mFaces[t];
		for(int i = 0; i < face->mNumIndices; i++) {
			int index = face->mIndices[i];
				curTexCoord = scene->mMeshes[0]->mTextureCoords[0][index];
				curTexCoord.x -= int(curTexCoord.x);
				curTexCoord.y -= int(curTexCoord.y);
				if( curTexCoord.x < 0 ) {
					curTexCoord.x += 1;
				}
				if( curTexCoord.y < 0 ) {
				...
				scene->mMeshes[0]->mTextureCoords[0][index] = curTexCoord;
		}
	}
}
